...
bool solveSimple(LL arr[N][N], LL remain[M], LL sum[M], LL *countRemain,LL unused[M+1],LL present[1+NN]){
	...
	reqVal = SUM-sum[k];
	if(sum[k] > SUM || reqVal >(NN) || present[reqVal]==1){
		return false;
	}
...
bool solve1(LL inp[N][N], LL inpRemain[M], LL inpSum[M], LL countRemain,LL inpUnused[M+1],LL inpPresent[1+NN]){

	// output(inp);				
	LL arr[N][N],i,j,k,l,curr=20, currRemain[3],countCurrRemain=0,x;
	LL remain[M], sum[M], unused[M+1], present[1+NN];
	memcpy(arr,inp,NN*sizeof(LL));
	...
	if(remain[curr] == 2){		//fill 1st element & recurse
		...
		for(i=0;i<countRemain;i++){
			...
			x= SUM - (sum[curr]+unused[i]); 
			if((sum[curr]+unused[i])>SUM || x>(NN) || (x == unused[i]) || present[x]==1 )continue;
		...
		for(i=0;i<countRemain;i++){
			...
			for(l=0;l<countRemain;l++){	
				...
				x= SUM - (sum[curr] + unused[l]); 
				if((sum[curr] + unused[l])>SUM || 
					x>(NN) || (x == unused[i]) || (x == unused[l]) 
...
void calc(){
	...
	LL arr[N][N],i,j,k,remain[M], sum[M], countRemain=0, unused[M+1], present[NN+1];	//max size of unused is M+1 = 15
	
	for(i=0;i<M;i++)remain[i] = sum[i] = 0;
	for(i=0;i<=(NN);i++)present[i]=0;
